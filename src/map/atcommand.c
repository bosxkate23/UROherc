	...
	*------------------------------------------*/
	...
	if (town < 0 || town >= ARRAYLENGTH(data)) {
		...
		if (strncmpi(map_name, "morroc", 4) == 0) { // Correct town name for 'morocc'
			town = 1;
		} else if (strncmpi(map_name, "lutiee", 3) == 0) { // Correct town name for 'xmas'
			town = 7;
			...
			town = 15;
		} else if (strncmpi(map_name, "market", 3) == 0 // Easy to remember alternatives to 'mall'
