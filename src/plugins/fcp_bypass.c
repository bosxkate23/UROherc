...
int skill_castend_nodamage_id_pre(struct block_list **src_, struct block_list **bl_, uint16 *skill_id_, uint16 *skill_lv_, int64 *tick_, int *flag_)
{
	...
	switch (skill_id) {
		...
		case SC_STRIPACCESSARY: {
			...
			// FCP
			// By pass FCP when using single strip skills by 5%(requires Glistening Coat).
			if (sd && tsc && sd->sc.data[SC_SOULLINK] && sd->sc.data[SC_SOULLINK]->val2 == SL_ROGUE && rand()%100 < 5 &&
